...
{
	return pSDL3Input->iGetAsyncKeyState(SDL_SCANCODE_LSHIFT) ||
		pSDL3Input->iGetAsyncKeyState(SDL_SCANCODE_Z) ||
		pSDL3Input->iGetAsyncKeyState(SDL_SCANCODE_X) ||
		pSDL3Input->iGetAsyncKeyState(SDL_SCANCODE_C);
}
...
void tune_remap(const Ivector& in_values, Ivector& out_values)
{
	...
	else if (pSDL3Input->iGetAsyncKeyState(SDL_SCANCODE_Z))
	{
		//strict by X
	...
	else if (pSDL3Input->iGetAsyncKeyState(SDL_SCANCODE_X))
	{
		//strict by Y
		out_values.x = 0;
	...
	else if (pSDL3Input->iGetAsyncKeyState(SDL_SCANCODE_C))
	{
		//strict by Z
		out_values.x = 0;
		out_values.y = 0;
...
void hud_draw_adjust_mode()
{
	...
	LPCSTR _text = NULL;
	if (pSDL3Input->iGetAsyncKeyState(SDL_SCANCODE_LSHIFT) && hud_adj_mode)
		_text =
...
void hud_adjust_mode_keyb(int dik)
{
	...
	if (pSDL3Input->iGetAsyncKeyState(SDL_SCANCODE_LCTRL))
	{
		if (dik == SDL_SCANCODE_KP_0)
			hud_adj_item_idx = 0;
		if (dik == SDL_SCANCODE_KP_1)
