...
void CUICustomMap::Initialize(shared_str name, LPCSTR sh_name)
{
	...
	{
		std::filesystem::path map_cfg_fn;
		FS.update_path(map_cfg_fn, "$game_levels$", std::filesystem::path(name.c_str()) / "level.ltx");
		levelIni = xr_new<CInifile>(map_cfg_fn.c_str());
	}
...
bool CUIGlobalMap::OnMouseAction(float x, float y, EUIMessages mouse_action)
{
	...
	if (mouse_action == WINDOW_MOUSE_MOVE && (FALSE == pSDL3Input->iGetAsyncBtnState(SDL_BUTTON_LEFT)))
	{
		if (MapWnd())
		{
			MapWnd()->Hint(MapName());
