...
class CControlPathBuilderBase : public CControl_ComBase
{
	...
	public:
		...
		inline const Fvector& position() const { return _position; }
		//inline		  Fvector	&position	()					{ return _position; }
		inline u32 node() const { return _node; }
		inline void set_node(u32 node_) { _node = node_; }
		inline void set_position(const Fvector& p) { _position.set(p); }
