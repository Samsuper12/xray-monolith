...
{
	if (this->object->HitMemory.get_last_hit_time() > time_state_started) return true;

	float dist = this->object->HitMemory.get_last_hit_position().distance_to(object->Position());
