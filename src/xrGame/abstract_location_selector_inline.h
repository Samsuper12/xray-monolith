...
inline CSelectorTemplate::CAbstractLocationSelector(CRestrictedObject* object)
{
...
TEMPLATE_SPECIALIZATION
inline CSelectorTemplate::~CAbstractLocationSelector()
{
}

TEMPLATE_SPECIALIZATION
inline void CSelectorTemplate::reinit(const _Graph* graph)
{
	m_failed = false;
...
TEMPLATE_SPECIALIZATION
inline _vertex_id_type CSelectorTemplate::get_selected_vertex_id() const
{
...
TEMPLATE_SPECIALIZATION
inline void CSelectorTemplate::set_evaluator(_VertexEvaluator* evaluator)
{
	m_evaluator = evaluator;
}

TEMPLATE_SPECIALIZATION
inline void CSelectorTemplate::set_query_interval(const u32 query_interval)
{
	m_query_interval = query_interval;
}

TEMPLATE_SPECIALIZATION
inline bool CSelectorTemplate::actual(const _vertex_id_type start_vertex_id, bool path_completed)
{
...
TEMPLATE_SPECIALIZATION
inline bool CSelectorTemplate::failed() const
{
	return (m_failed);
}

TEMPLATE_SPECIALIZATION
inline bool CSelectorTemplate::used() const
{
	return (m_evaluator && m_graph);
}

TEMPLATE_SPECIALIZATION
inline void CSelectorTemplate::select_location(const _vertex_id_type start_vertex_id, bool path_completed)
{
	if (used() && (((m_last_query_time + m_query_interval) <= Device.dwTimeGlobal) || path_completed))
	{
		perform_search(start_vertex_id);
...
TEMPLATE_SPECIALIZATION
inline void CSelectorTemplate::perform_search(const _vertex_id_type vertex_id)
{
...
TEMPLATE_SPECIALIZATION
inline void CSelectorTemplate::set_dest_path(xr_vector<_vertex_id_type>& path)
{
	m_path = &path;
}

TEMPLATE_SPECIALIZATION
inline void CSelectorTemplate::set_dest_vertex(_vertex_id_type& vertex_id)
{
	dest_vertex_id = &vertex_id;
}

TEMPLATE_SPECIALIZATION
inline void CSelectorTemplate::before_search(_vertex_id_type& vertex_id)
{
}

TEMPLATE_SPECIALIZATION
inline void CSelectorTemplate::after_search()
{
}
