...
TEMPLATE_SPECIALIZATION
inline bool CFixedVertexManager::is_opened(const CGraphVertex& vertex) const
{
	return (!!vertex.opened());
}

TEMPLATE_SPECIALIZATION
inline bool CFixedVertexManager::is_visited(const _index_type& vertex_id) const
{
...
TEMPLATE_SPECIALIZATION
inline bool CFixedVertexManager::is_closed(const CGraphVertex& vertex) const
{
	return (is_visited(vertex) && !is_opened(vertex));
}

TEMPLATE_SPECIALIZATION
inline typename CFixedVertexManager::CGraphVertex&CFixedVertexManager::get_node(const _index_type& vertex_id) const
{
	VERIFY(vertex_id < m_max_node_count);
...
TEMPLATE_SPECIALIZATION
inline typename CFixedVertexManager::CGraphVertex&CFixedVertexManager::create_vertex(
...
TEMPLATE_SPECIALIZATION
inline void CFixedVertexManager::add_opened(CGraphVertex& vertex)
{
	vertex._opened = 1;
}

TEMPLATE_SPECIALIZATION
inline void CFixedVertexManager::add_closed(CGraphVertex& vertex)
{
	vertex._opened = 0;
}

TEMPLATE_SPECIALIZATION
inline _path_id_type CFixedVertexManager::current_path_id() const
{
