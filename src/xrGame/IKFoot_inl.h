inline Fvector& CIKFoot::ToePosition(Fvector& toe_position) const
{
...
inline Fvector& CIKFoot::HeelPosition(Fvector& heel_position) const
{
...
inline Fvector& CIKFoot::FootNormal(Fvector& foot_normal) const
{
	return get_local_vector(foot_normal, m_foot_normal);
}

inline Fvector& CIKFoot::get_local_vector(Fvector& v, const local_vector& lv) const
{
	return get_local_vector(ref_bone(), v, lv);
}

inline Fvector& CIKFoot::get_local_vector(u16 bone, Fvector& v, const local_vector& lv) const
{
	if (bone == lv.bone)
	{
		v.set(lv.v);
