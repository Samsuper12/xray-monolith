...
public:
	...
	inline void set_ref_bone(u16 ref_bone) { m_ref_bone = ref_bone; }
	void set_ref_bone();
	u16 get_ref_bone(const Fmatrix& foot_transform, const Fmatrix& toe_transform) const;
public:
	inline Fvector& ToePosition(Fvector& toe_position) const;
	inline Fvector& HeelPosition(Fvector& heel_position) const;
	inline u16 ref_bone() const { return m_ref_bone; }
	Fmatrix& ref_bone_to_foot(Fmatrix& foot, const Fmatrix& ref_bone) const;
	Fmatrix& ref_bone_to_foot(Fmatrix& ref_bone) const;
private:
public:
	inline Fvector& FootNormal(Fvector& foot_normal) const;
private:
	inline Fvector& get_local_vector(Fvector& v, const local_vector& lv) const;
	inline Fvector& get_local_vector(u16 bone, Fvector& v, const local_vector& lv) const;
...
private:
	...
	inline bool make_shift(Fmatrix& xm, const Fvector& cl_point, bool collide, const Fplane& p,
	                   const Fvector& pick_dir) const;

private:
	void set_toe(u16 bones[4]);
public:
	inline IKinematics* Kinematics() const { return m_K; }
