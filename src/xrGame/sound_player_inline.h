...
inline void CSoundPlayer::set_sound_mask(u32 sound_mask)
{
	m_sound_mask = sound_mask;
	remove_inappropriate_sounds(m_sound_mask);
}

inline void CSoundPlayer::remove_active_sounds(u32 sound_mask)
{
...
inline const xr_vector<CSoundPlayer::CSoundSingle>& CSoundPlayer::playing_sounds() const
{
	return (m_playing_sounds);
}

inline u32 CSoundPlayer::active_sound_count(bool only_playing) const
{
	u32 count = 0;
...
inline ref_sound* CSoundPlayer::CSoundCollection::add(ESoundTypes type, LPCSTR name) const
{
...
inline const CSoundPlayer::SOUND_COLLECTIONS& CSoundPlayer::objects() const
{
	return (m_sounds);
}

inline bool CSoundPlayer::active_sound_type(u32 synchro_mask) const
{
	xr_vector<CSoundSingle>::const_iterator I = m_playing_sounds.begin();
	xr_vector<CSoundSingle>::const_iterator E = m_playing_sounds.end();
	for (; I != E; ++I)
