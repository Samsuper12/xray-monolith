...
protected:
	...
	inline void nearest(const Fvector& position, float radius, xr_vector<_object_type*>& objects, CQuadNode* node,
...
public:
	inline CQuadTree(const Fbox& box, float min_cell_size, u32 max_node_count, u32 max_list_item_count);
	virtual ~CQuadTree();
	inline void clear();
	inline void insert(_object_type* object);
	inline _object_type* remove(const _object_type* object);
	inline _object_type* find(const Fvector& position) const;
