...
inline bool CSightManager::use_torso_look() const
{
	if (!m_actions.empty())
...
inline bool CSightManager::turning_in_place() const
{
	return (m_turning_in_place);
}

inline bool CSightManager::enabled() const
{
	return (m_enabled);
}

template <typename T1, typename T2, typename T3>
inline void CSightManager::setup(T1 _1, T2 _2, T3 _3)
{
	setup(CSightAction(_1, _2, _3));
}

template <typename T1, typename T2>
inline void CSightManager::setup(T1 _1, T2 _2)
{
	setup(CSightAction(_1, _2));
}

template <typename T1>
inline void CSightManager::setup(T1 _1)
{
	setup(CSightAction(_1));
}

inline Fmatrix const& CSightManager::current_spine_rotation() const
{
	return (m_current.m_spine.m_rotation);
}

inline Fmatrix const& CSightManager::current_shoulder_rotation() const
{
	return (m_current.m_shoulder.m_rotation);
}

inline Fmatrix const& CSightManager::current_head_rotation() const
{
	return (m_current.m_head.m_rotation);
}

inline void CSightManager::bone_aiming()
{
	m_animation_id = "";
