...
public:
	...
	inline EBodyState body_state_combat_override(EWorldOperators wo, EBodyState body_state);
#endif
...
public:
	inline SBoneRotation const& head_orientation() const;
	inline Fvector const* desired_position() const;
	inline Fvector const* desired_direction() const;
	inline EBodyState const& body_state() const;
	inline EBodyState const& target_body_state() const;
	inline EMovementType const& movement_type() const;
	inline EMentalState const& mental_state() const;
	inline EMentalState const& target_mental_state() const;
	inline EPathType const& path_type() const;
	inline EDetailPathType const& detail_path_type() const;
	inline bool use_desired_position() const;
	inline bool use_desired_direction() const;
	inline EMovementType const& target_movement_type() const;
	inline CAI_Stalker& object() const;
	inline stalker_movement_params const& current_params() const;
	inline stalker_movement_params& target_params();
...
private:
	inline void setup_head_speed(stalker_movement_params& movement_params);
