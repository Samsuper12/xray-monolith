...
{
	return (inherited::actual(this->m_object->object().ai_location().game_vertex_id(), inherited::dest_vertex_id()));
}
...
{
	if (inherited::path().empty() || (this->m_intermediate_index >= (_vertex_id_type)inherited::path().size() - 1))
