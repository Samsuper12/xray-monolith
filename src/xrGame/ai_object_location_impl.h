...
inline void CAI_ObjectLocation::init()
{
...
inline void CAI_ObjectLocation::game_vertex(CVertex const* game_vertex)
{
...
inline const CGameGraph::CVertex* CAI_ObjectLocation::game_vertex() const
{
	VERIFY(ai().game_graph().valid_vertex_id(m_game_vertex_id));
	return (ai().game_graph().vertex(m_game_vertex_id));
}

inline void CAI_ObjectLocation::level_vertex(CLevelVertex const* level_vertex)
{
