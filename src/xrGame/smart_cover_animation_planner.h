...
class animation_planner :
	...
	public:
		...
		inline u32 const& time_object_hit() const;
		inline u32 const& loophole_value() const;
		inline void decrease_loophole_value(u32 const& value);
		inline u32 const& last_transition_time() const;
		inline void last_transition_time(u32 const& value);
		inline CPropertyStorage* property_storage();
		inline shared_str cName() const;
		inline u32 default_idle_interval();
		inline u32 default_lookout_interval();
		inline float const& idle_min_time() const;
		inline void idle_min_time(float const& value);
		inline float const& idle_max_time() const;
		inline void idle_max_time(float const& value);
		inline float const& lookout_min_time() const;
		inline void lookout_min_time(float const& value);
		inline float const& lookout_max_time() const;
		inline void lookout_max_time(float const& value);
		inline bool const& stay_idle() const;
		inline void stay_idle(bool const& value);
		inline u32 const& last_idle_time() const;
		inline void last_idle_time(u32 const& value);
		inline u32 const& last_lookout_time() const;
		inline void last_lookout_time(u32 const& value);
