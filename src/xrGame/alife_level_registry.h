...
public:
	inline CALifeLevelRegistry(const GameGraph::_LEVEL_ID& level_id);
	inline void add(CSE_ALifeDynamicObject* tpALifeDynamicObject);
	inline void remove(CSE_ALifeDynamicObject* tpALifeDynamicObject, bool no_assert = false);
	template <typename _update_predicate>
	inline void update(const _update_predicate& predicate, bool const iterate_as_first_time_next_time);
	inline GameGraph::_LEVEL_ID level_id() const;
