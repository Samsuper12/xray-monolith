...
float OLES_SUN_LIMIT_27_01_07; //	actually sun_far
...
public :
	...
	virtual void Execute(LPCSTR args)
	{
		...
		//vkRenderDeviceRender::Instance().ResourcesGetMemoryUsage(m_base, c_base, m_lmaps, c_lmaps);
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		if (v < ps_r2_dof.y + 0.1f)
		{
			...
			Msg("~ Valid arguments: %s", "float value greater or equal to r2_dof_focus+0.1");
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		{
			Msg("~ Invalid syntax in call to '%s'", cName);
			Msg("~ Valid arguments: %s", "float value less or equal to r2_dof_focus-0.1");
			Console->Execute("r2_dof_focus");
		}
		else
		{
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		{
			Msg("~ Invalid syntax in call to '%s'", cName);
			Msg("~ Valid arguments: %s", "float value less or equal to r2_dof_far-0.1");
			Console->Execute("r2_dof_far");
		}
		else if (v < ps_r2_dof.x + 0.1f)
		{
			Msg("~ Invalid syntax in call to '%s'", cName);
			Msg("~ Valid arguments: %s", "float value greater or equal to r2_dof_far-0.1");
...
public:
	...
	{
		//RImplementation.Models->dump();
		//vkRenderDeviceRender::Instance().Resources->Dump(false);
	}
};
