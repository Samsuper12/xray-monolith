...
protected:
	...
	inline void add(const CLASS_ID& clsid, LPCSTR script_clsid);
...
#else
	inline		const CObjectItemAbstract	*item							(const CLASS_ID &clsid, bool no_assert) const;
#endif
...
public:
	...
	inline SERVER_BASE_CLASS* server_object(const CLASS_ID& clsid, LPCSTR section) const;
#else
	inline		SERVER_BASE_CLASS			*server_object					(const CLASS_ID &clsid, LPCSTR section) const;
#endif
